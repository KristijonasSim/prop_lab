# VIX.change5.EURUSD.h20 — feed screen
...
CBOE equity volatility index. Equity stress sends allocators to the safe haven; the seller is a risk-parity book cutting equity. High VIX predicts gold UP, and by the same mechanism crypto DOWN - which is what the cross-market re-run is for.

Source: CBOE. CBOE equity volatility index.
...
| lag | 1 (Settlement value, readable the following session.) |
| market | EURUSD |
