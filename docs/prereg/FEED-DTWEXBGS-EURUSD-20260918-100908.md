# DTWEXBGS.change5.EURUSD.h10 — feed screen

Written 2026-09-18T10:09:08+00:00 by
...
Broad trade-weighted dollar index. Gold is quoted in dollars; a stronger dollar makes the same ounce cost fewer of them. High DTWEXBGS predicts gold DOWN.
...
| market | EURUSD |
| hold | 10 trading days, open to open |
| direction | -1 |
...
EURUSD round trip 0.19 bps at `mixed` execution. The effect
must clear **0.37 bps** (2.0x) between the top and bottom bucket.

## Kill criterion — fixed before the number is known

Any one of: fewer than 40 independent events; effect under
0.37 bps; mean and median disagreeing in sign; |rho| under 0.8;
p >= 0.05 against the shuffle. **First failure ends it — no second look, no
tuning of the window, no other market rescuing it.**

## Expected events

92 independent, from `core.screen.independent_events`.
