# DTWEXBGS.change20.XAUUSD.h5 — feed screen

Written 2026-09-18T11:52:29+00:00 by
...
Gold is dollar-quoted; a rising broad trade-weighted dollar mechanically raises gold's local-currency price for non-US buyers (Indian/Chinese jewelry demand, EM central banks), and those price-sensitive buyers step back, leaving USD-based sellers in control.

Source: FRED. Broad trade-weighted dollar index.
...
|---|---|
| feed | `DTWEXBGS` |
| transform | `change` window 20 |
| lag | 1 (FRED daily, published next business day.) |
| market | XAUUSD |
...
XAUUSD round trip 1.06 bps at `mixed` execution. The effect
must clear **2.13 bps** (2.0x) between the top and bottom bucket.

## Kill criterion — fixed before the number is known

Any one of: fewer than 40 independent events; effect under
2.13 bps; mean and median disagreeing in sign; |rho| under 0.8;
p >= 0.05 against the shuffle. **First failure ends it — no second look, no
tuning of the window, no other market rescuing it.**

## Expected events

50 independent, from `core.screen.independent_events`.
