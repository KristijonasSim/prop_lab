# DTWEXBGS.change5.GBPUSD.h10 — feed screen

Written 2026-09-18T10:59:11+00:00 by
...
GBPUSD round trip 0.41 bps at `mixed` execution. The effect
must clear **0.82 bps** (2.0x) between the top and bottom bucket.

## Kill criterion — fixed before the number is known

Any one of: fewer than 40 independent events; effect under
0.82 bps; mean and median disagreeing in sign; |rho| under 0.8;
p >= 0.05 against the shuffle. **First failure ends it — no second look, no
tuning of the window, no other market rescuing it.**

## Expected events

88 independent, from `core.screen.independent_events`.
